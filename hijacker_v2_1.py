...
import socket  # Módulo para resolver IPs de domínios
...
# Função para obter IPs IPv4 de github.com
def get_github_ips():
    ips = set()
    try:
        for info in socket.getaddrinfo('github.com', 443):
            if info[0] == socket.AF_INET:  # Apenas IPv4
                ips.add(info[4][0])
    except socket.gaierror as e:
        print(f"Erro ao resolver IPs de github.com: {e}")
        sys.exit(1)
    if not ips:
        print("Não foram encontrados IPs para github.com")
        sys.exit(1)
    return ips

# Função para aplicar regras de iptables para redirecionamento, excluindo IPs de github.com
def apply_iptables_rules():
    ips = get_github_ips()
    print(f"IPs de github.com excluídos do hijacking: {ips}")
    
    try:
        subprocess.run(["sudo", "iptables", "-t", "nat", "-F"], check=True, capture_output=True, text=True)
        print("Regras existentes na tabela nat limpas.")
    except subprocess.CalledProcessError as e:
        print(f"Erro ao limpar regras existentes: {e.stderr}")
        sys.exit(1)
    
    custom_chains = ["HTTP_HIJACK", "HTTPS_HIJACK"]
    for chain in custom_chains:
        try:
            subprocess.run(["sudo", "iptables", "-t", "nat", "-F"], check=False, capture_output=True, text=True) 
            subprocess.run(["sudo", "iptables", "-t", "nat", "-X"], check=False, capture_output=True, text=True)
        except subprocess.CalledProcessError as e:
            pass
    # Comandos para criar chains personalizados e regras de exclusão
    commands = [
        "iptables -t nat -N HTTP_HIJACK",
        "iptables -t nat -A OUTPUT -p tcp --dport 80 -j HTTP_HIJACK",
        "iptables -t nat -N HTTPS_HIJACK",
        "iptables -t nat -A OUTPUT -p tcp --dport 443 -j HTTPS_HIJACK",
    ]

    # Adiciona regras de RETURN para cada IP no chain HTTP_HIJACK
    for ip in ips:
        commands.append(f"iptables -t nat -A HTTP_HIJACK -d {ip} -p tcp -j RETURN")

    # Adiciona a regra de REDIRECT no final do chain HTTP_HIJACK
    commands.append("iptables -t nat -A HTTP_HIJACK -p tcp -j REDIRECT --to-ports 8080")

    # Adiciona regras de RETURN para cada IP no chain HTTPS_HIJACK
    for ip in ips:
        commands.append(f"iptables -t nat -A HTTPS_HIJACK -d {ip} -p tcp -j RETURN")

    # Adiciona a regra de REDIRECT no final do chain HTTPS_HIJACK
    commands.append("iptables -t nat -A HTTPS_HIJACK -p tcp -j REDIRECT --to-ports 8443")
    ...
    print("Regras de hijacking aplicadas! Todo tráfego HTTP/HTTPS será redirecionado para os servidores locais, exceto para github.com.")

# Função para limpar regras de iptables ao encerrar
def cleanup_iptables():
    try:
        # Remove as referências nos chains OUTPUT
        subprocess.run(["sudo", "iptables", "-t", "nat", "-D", "OUTPUT", "-p", "tcp", "--dport", "80", "-j", "HTTP_HIJACK"], check=False)
        subprocess.run(["sudo", "iptables", "-t", "nat", "-D", "OUTPUT", "-p", "tcp", "--dport", "443", "-j", "HTTPS_HIJACK"], check=False)
        
        # Flush e deleta os chains personalizados
        subprocess.run(["sudo", "iptables", "-t", "nat", "-F", "HTTP_HIJACK"], check=False)
        subprocess.run(["sudo", "iptables", "-t", "nat", "-X", "HTTP_HIJACK"], check=False)
        subprocess.run(["sudo", "iptables", "-t", "nat", "-F", "HTTPS_HIJACK"], check=False)
        subprocess.run(["sudo", "iptables", "-t", "nat", "-X", "HTTPS_HIJACK"], check=False)
        
        # Flush geral da tabela nat para consistência com o original
